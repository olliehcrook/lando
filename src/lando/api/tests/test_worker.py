...
from lando.main.models.jobs import JobAction
...
@pytest.mark.django_db
def test_Worker_fails_abandoned_job_on_start(
    landing_worker_instance, make_landing_job, repo_mc
):
    repo = repo_mc(SCMType.GIT)
    ...
    LandingWorker(worker_model, with_ssh=False).start(max_loops=-1)

    job.refresh_from_db()
    assert job.status == JobStatus.FAILED
    assert job.attempts == 1
    assert job.error == "The worker stopped while processing this job."
